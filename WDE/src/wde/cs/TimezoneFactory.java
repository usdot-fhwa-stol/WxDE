...
public class TimezoneFactory {
    ...
    private static String TIMEZONE_QUERY = "SELECT id, rawOffset, tzName, " +
            ...
            "endMonth, endDay, endDayOfWeek, endTime, dstSavings FROM conf.timezone";
    ...
    TimezoneFactory(Connection iConnection) {
        ...
        try {
            ...
            ps = iConnection.prepareStatement(TIMEZONE_QUERY);
            rs = ps.executeQuery();

            // save the timestamp format strings by their id
            while (rs.next())
                m_oTimezones.add(new TimezoneRecord(rs));
    ...
    private class TimezoneRecord implements Comparable<TimezoneRecord> {
         ...
         * set.
         *
         * @param rs The result set to gather the data from. Assumes
         *           the query has been made and assigned to the result set.
         */
        private TimezoneRecord(ResultSet rs) {
            try {
                m_nId = rs.getInt(1);
                m_nRawOffset = rs.getInt(2);
                m_sName = rs.getString(3);
                m_nStartMonth = introspect(rs.getString(4));
                m_nStartDay = rs.getInt(5);
                m_nStartDayOfWeek = introspect(rs.getString(6));
                m_nStartTime = rs.getInt(7);
                m_nEndMonth = introspect(rs.getString(8));
                m_nEndDay = rs.getInt(9);
                m_nEndDayOfWeek = introspect(rs.getString(10));
                m_nEndTime = rs.getInt(11);
                m_nDstSavings = rs.getInt(12);
            } catch (Exception oException) {
                oException.printStackTrace();
            }
        }

...
public class Polyline implements Iterator<PolylinePart>
{
	...
	private int[] m_oParts = new int[1]; // typical part count is 1
	...
	public void set(int[] oPolyline)
	{
		...
		int nPartCount = oPolyline[4] + 1;
		if (m_oParts.length != nPartCount)
			m_oParts = new int[nPartCount]; // create array to hold point offsets
	...
	public PolylinePart next()
	{
		...
		int nEnd = m_oPolyline.length; // default to end of polyline data
	...
	{
		if (oMidPoint == null || oMidPoint.length < 2)
			return;
	
		m_nPartIndex = 0; // reset part iterator
		if (hasNext()) // improve algorithm--this is an end point
		{
			PolylinePart oPart = next();
			if (oPart.hasNext())
				System.arraycopy(oMidPoint, 0, oPart.next(), 0, 2);
		}
	}


	/**
	 * Returns the posted speed limit for the polyline representing a roadway.
	 *
	 * @return the posted speed limit in meters per second
	 */
	public int getSpeed()
	{
		return 27; // 27 m/s is approximately 60 mph
	}

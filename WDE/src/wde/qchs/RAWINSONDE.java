...
import wde.obs.IObs;
import wde.qchs.RAWINSONDE.CachedLatLon;
import wde.qchs.algo.HeightTemp;
import wde.util.*;
 ...
 * Weather Service balloon soundings along with the originating platform data.
...
public class RAWINSONDE implements Runnable, ILockFactory<CachedLatLon> {
     ...
     * load the platform list from a file and get the current list of sounding
     * files from the ftp server. Depending on how long it will be until the
     * scheduler will update the platform list with new readings, it may or may
     * not perform an initial update.
     */
    private RAWINSONDE() {
        Config oConfig = ConfigSvc.getInstance().getConfig(this);

        // the reanalysis data changes rarely and can be loaded once
        ...
        Scheduler.getInstance().schedule(this,
                ...
                oConfig.getInt("schedulerPeriod", 900), true);
    }
    ...
    /**
     * Initializes the platform list using values loaded from a CSV file.
    ...
    private void restore() {
        ...
        } catch (Exception oException) {//Failed to load
        }
    }
    ...
    private void save() {
        try {
            ...
            oFileWriter.close();
        } catch (Exception oException) {//failed to save file
        }
    }
    ...
    public void getCurrentFileList() {
        ...
        try {
            ...
            oReader.close();
        } catch (Exception oException) {// Error on a CharTokenizer.NextSet() call.  List may not have
        }// been fully updated.
    }
                ...
                if (m_oFtpConn.open(m_sFtpDirectory + oUpdateFile.m_sFileName)) {
                    ...
                    while (oCharTokenizer.nextSet()) {
                        while (oCharTokenizer.hasTokens()) {
                            ...
                            //a platform record.  Go back to looking for 'TTAA'
                            //if it is.
                            ...
                            switch (nState) {
                                ...
                                case 2: // Current Token = Timestamp
                                    ...
                                    nState = 3;
                                    break;
                                case 3: // Current Token = Platform Code
                                    m_oSearchSoundingRecord.m_sPlatformCode =
     ...
     * readings. Once all the readings are taken, the current platform list--
    ...
    public boolean getHeightTemp(HeightTemp oHeightTemp, IObs iObs) {
        ...
        oCachedLatLon.m_lLat = iObs.getLatitude();
        oCachedLatLon.m_lLon = iObs.getLongitude();
        ...
        if (nLookupIndex >= 0) {
            ...
            oCachedLatLon.m_nPrimaryPlatformIndex = -1;
            oCachedLatLon.m_nSecondaryPlatformIndex = -1;
            oCachedLatLon.m_sPrimaryPlatformCode = null;
            oCachedLatLon.m_sSecondaryPlatformCode = null;
            //Loop through all the platforms
            int nIndex = m_oSoundingRecordList.size();
            while (nIndex-- > 0) {
                SoundingRecord oSoundingRecord = m_oSoundingRecordList.get(nIndex);
                long lDeltaX = iObs.getLatitude() - oSoundingRecord.m_nLatitude;
                long lDeltaY = iObs.getLongitude() - oSoundingRecord.m_nLongitude;
                long lDistance = lDeltaX * lDeltaX + lDeltaY * lDeltaY;
                //See if this platform is the closest platform yet
                if (lDistance < lPrimary || lPrimary == Long.MIN_VALUE) {
                    ...
                    oCachedLatLon.m_nSecondaryPlatformIndex =
                            oCachedLatLon.m_nPrimaryPlatformIndex;
                    //Store new primary index
                    oCachedLatLon.m_nPrimaryPlatformIndex = nIndex;
                    //Move current primary code to secondary
                    oCachedLatLon.m_sSecondaryPlatformCode =
                            oCachedLatLon.m_sPrimaryPlatformCode;
                    //Store new code as primary
                    oCachedLatLon.m_sPrimaryPlatformCode =
                            oSoundingRecord.m_sPlatformCode;
                    ...
                    oCachedLatLon.m_nSecondaryPlatformIndex = nIndex;
                    oCachedLatLon.m_sPrimaryPlatformCode =
                            oSoundingRecord.m_sPlatformCode;
                }
            }
            //If a platform was looked up successfully store the result in the cache
            //and return the validity of the platform's current readings,
        ...
        if (bReturn) {
            ...
            //readings for the closest platform or, if the values for the
            ...
            synchronized (m_oCalendar) {
                ...
                m_oReanalysisSearch.m_nLat = iObs.getLatitude();
                m_oReanalysisSearch.m_nLon = iObs.getLongitude();
        ...
        return bReturn; // No valid platform found
    }
    ...
    /**
     * The CachedLatLon class is used to store the two closest platforms
    ...
    public class CachedLatLon implements Comparable<CachedLatLon> {
        ...
        public int m_lLon;
        /**
         * platformCode of the closest platform
         */
        public String m_sPrimaryPlatformCode = null;
        /**
         * Location in m_oActiveStationList of the closest platform
         */
        public int m_nPrimaryPlatformIndex = -1;
        /**
         * PlatformCode of the second closest platform
         */
        public String m_sSecondaryPlatformCode = null;
        /**
         * Location in the m_oActiveStationList of the second closest platform
         */
        public int m_nSecondaryPlatformIndex = -1;
        /**
        ...
        public CachedLatLon(CachedLatLon oCachedLatLon) {
            ...
            this.m_nPrimaryPlatformIndex = oCachedLatLon.m_nPrimaryPlatformIndex;
            this.m_nSecondaryPlatformIndex = oCachedLatLon.m_nSecondaryPlatformIndex;
            this.m_sPrimaryPlatformCode = new String(oCachedLatLon.m_sPrimaryPlatformCode);
            this.m_sSecondaryPlatformCode = new String(oCachedLatLon.m_sSecondaryPlatformCode);
        }


        /**
         * @return true if the primary cached platform is still in the list and
         * it currently has valid height and temperature readings
         */
        public boolean verifyPrimaryValues() {
            return verifyValues(m_nPrimaryPlatformIndex, m_sPrimaryPlatformCode);
        }


        /**
         * @return true if the secondary cached platform is still in the list and
         * it currently has valid height and temperature readings
         */
        public boolean verifySecondaryValues() {
            if (m_sSecondaryPlatformCode == null)
                return false;

            return verifyValues(m_nSecondaryPlatformIndex, m_sSecondaryPlatformCode);
        }


        /**
         * @param nIndex Index of the cached platform in m_oActiveStationList
         * @param sCode  Expected platform code.  Used to verify the platform
         *               at nIndex is the correct platform, and then to lookup the new index
         *               if the code for the platform at nIndex does not match sCode
         * @return true if two conditions are met:
         * <BR>1. The code of the platform at nIndex matches sCode, or if they
         * don't match the platform's new index can be looked up in
         * m_oSoundingRecordList
         * <BR>2. The current readings for the cached platform are valid ( not
         * equal to NaN and taken within the last 12 hours)
         */
        private boolean verifyValues(int nIndex, String sCode) {
            ...
            if (nIndex >= 0 && nIndex < m_oSoundingRecordList.size()) {
                m_oSoundingRecord = m_oSoundingRecordList.get(nIndex);

                //Check if the platform at the cached index has changed
                if (sCode.compareTo(m_oSoundingRecord.m_sPlatformCode) == 0) {
                    //If the platform hasn't changed, return the validity of the
            ...
            //The cached platform's index has changed, so set up the
            //SoundingRecord object to lookup the new index
            m_oSoundingRecord.m_sPlatformCode = sCode;
            //Get the platform's new index
            int nLookupIndex = Collections.binarySearch(m_oSoundingRecordList,
                    m_oSoundingRecord);
            //Make sure the platform is still in the list
            if (nLookupIndex < 0) {
                //If the cached platform can't be found in the list, remove
            ...
            if (nIndex == m_nPrimaryPlatformIndex)
                m_nPrimaryPlatformIndex = nLookupIndex;
            else
                m_nSecondaryPlatformIndex = nLookupIndex;
            //Get the cached StationRecord using the new index
            m_oSoundingRecord = m_oSoundingRecordList.get(nLookupIndex);
            //return the validity of the platform's current readings
            return lCutOff <= m_oSoundingRecord.m_lTimeStamp
                    && !Double.isNaN(m_oSoundingRecord.m_dHeight)
                    && !Double.isNaN(m_oSoundingRecord.m_dTemperature);
        }
     ...
     * originating platform, and the methods used to decode values from the NWS
     * sounding files
     */
    public class SoundingRecord implements Comparable<SoundingRecord> {
        ...
        /**
         * platform associated with the reading
         */
        public String m_sPlatformCode = "";
        /**
         * Latitude of the platform associated with the reading
         */
        public int m_nLatitude = 0;
        /**
         * Longitude of the platform associated with the reading
         */
        ...
        public SoundingRecord(String[] sCols) {
            if (sCols.length > 0)
                m_sPlatformCode = sCols[0];
            if (sCols.length > 1)
                m_nLatitude = MathUtil.toMicro(Double.parseDouble(sCols[1]));
            if (sCols.length > 2)
                m_nLongitude = MathUtil.toMicro(Double.parseDouble(sCols[2]));
            if (sCols.length > 3)
        ...
        public String toString() {
            ...
            return m_sPlatformCode
                    + "," + Double.toString(MathUtil.fromMicro(m_nLatitude))
                    + "," + Double.toString(MathUtil.fromMicro(m_nLongitude))
         ...
         * @return true if the platform codes are equal
         */
        public boolean equals(SoundingRecord oComparisonRecord) {
            return this.m_sPlatformCode.equals(oComparisonRecord.m_sPlatformCode);
        }
        ...
        public int compareTo(SoundingRecord oSoundingRecord) {
            return
                    this.m_sPlatformCode.compareTo(oSoundingRecord.m_sPlatformCode);
        }
    }
    ...
    private class ReanalysisRecord implements Comparable<ReanalysisRecord> {
        ...
        private ReanalysisRecord(String sLine) {
            ...
            m_nLat = MathUtil.toMicro(Double.parseDouble(sCols[0]));
            m_nLon = MathUtil.toMicro(Double.parseDouble(sCols[1]) - 360.0);

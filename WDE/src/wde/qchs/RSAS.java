...
public class RSAS implements Runnable//, singleton
{
    ...
    private RSAS() {
        ...
        Scheduler.getInstance().schedule(this,
                ...
                oConfig.getInt("schedulerPeriod", 3600), true);
    }
    ...
    public static void main2(String args[]) throws Exception {
        RSAS.getInstance().getReadings(581, MathUtil.toMicro(34.4), MathUtil.toMicro(-90.40));
    }
    ...
    public double[] getReadings(int nObsID, int nLat, int nLon) {
        try {
            ...
            ProjectionPointImpl oPoint =
                    (ProjectionPointImpl) m_oProjection.latLonToProj(MathUtil.fromMicro(nLat),
...
//			if(!m_oProjection.getDefaultMapArea().contains(dNativeX, dNativeY))
            if (!m_oProjection.getDefaultMapArea().contains(oPoint))
                return null;
            ...
            for (nX = 0; nX < POINT_COUNT_X; ++nX)
                if (oXPoints.getDouble(nX) > oPoint.getX())//dNativeX)
                    break;
            for (nY = 0; nY < POINT_COUNT_Y; ++nY)
                if (oYPoints.getDouble(nY) > oPoint.getY())//dNativeY)
                    break;
            //Since the second of the two bounding points is currently stored,
            //see if the first one is closer
            if (oPoint.getX() - oXPoints.getDouble(nX - 1) <
                    oXPoints.getDouble(nX) - oPoint.getX())
                --nX;
            if (oPoint.getY() - oYPoints.getDouble(nY - 1) <
                    oYPoints.getDouble(nY) - oPoint.getY())
                --nX;
            //Now that the x, y coordinates have been found, just return the values.
            switch (nObsID) {
                case 5733:
                    return getValues(m_dTemperature, nX, nY);
                case 581:
                    return getValues(m_dHumidity, nX, nY);
                case 554:
                    return getValues(m_dPressure, nX, nY);
                case 56104:
                    return getValues(m_dWindMagnitude, nX, nY);
                default:
                    return null;
            }
        } catch (Exception oException) {

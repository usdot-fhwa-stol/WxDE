...
public class ClimateRecords extends DbCache<ClimateRecord> {
    ...
    private ClimateRecords() {
        ...
        m_sQuery = "SELECT obsTypeId, month, lat, lon, " +
                "lowerValue, upperValue, avgValue FROM conf.climateRecord";
...
//		oPrintWriter.println("</td></tr>");
     ...
     * </p>
     *
     * @param rs             set containing the {@code ClimateRecord} data.
     * @param oClimateRecord the record whose parameters are to be modified.
     */
    protected void setParameters(ResultSet rs, ClimateRecord oClimateRecord) {
        try {
            oClimateRecord.m_nObsTypeId = rs.getInt(1);
            // month lookups are zero-based
            oClimateRecord.m_nMonth = rs.getInt(2) - 1;
            oClimateRecord.m_nLat = MathUtil.toMicro(rs.getDouble(3));
            oClimateRecord.m_nLon = MathUtil.toMicro(rs.getDouble(4));
            oClimateRecord.m_dMin = rs.getDouble(5);
            oClimateRecord.m_dMax = rs.getDouble(6);
            oClimateRecord.m_dAvg = rs.getDouble(7);
     ...
     * @return <pre> true if the min, max, and average values of {@code oLhs}  </pre>
     ...
     * to the {@link wde.util.threads.StripeLock} Mutex.
     * <p>
     ...
     * @see wde.util.threads.StripeLock
     */
    @Override

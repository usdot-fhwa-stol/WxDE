...
//      fc_day.tm_mday = fc_gtm.tm_mday;
     ...
     *  dPrecipType      array containing the forecasted precipitation types (0 = none, 1 = rain, 2 = snow)
     *  dRoadCond	 array containing the observed road condition (we initialize the 1st based off of precipitation and air temp. after that we use the METRo results as the input, the values are listed below)
     ...
     *
     */
    JNIEXPORT void JNICALL Java_wde_comp_DoMetroWrapper_doMetroWrapper
      (JNIEnv *env, jobject thisObj, jint bBridge, jdouble dMLat, jdouble dMLon,
            ...
            jdoubleArray dPrecipType, jdoubleArray dRoadCond, jdoubleArray dCloudCover,
            jlongArray lOutRoadCond, jdoubleArray dOutRoadTemp, jdoubleArray dOutSubSurfTemp)
    {
        ...
        jdouble *cdPrecipType = (*env)->GetDoubleArrayElements(env, dPrecipType, NULL);
        jdouble *cdRoadCond = (*env)->GetDoubleArrayElements(env, dRoadCond, NULL);
...
//            dTimeO[i] = dTimeO[i - 1] + (1.0/120.0);  //fill in values for the obs times for every 30 seconds
        ...
        interp(dRC, nLenObservation, cdRoadCond, nObservationHrs, metroTimesPerHour, 1);
...
//        }
        ...
        interp(dTYP, nNbrTimeSteps, cdPrecipType, nForecastHrs, metroTimesPerHour, 1);   
...
//        }
        ...
        (*env)->ReleaseDoubleArrayElements(env, dPrecipType, cdPrecipType, 0);
        (*env)->ReleaseDoubleArrayElements(env, dRoadCond, cdRoadCond, 0);

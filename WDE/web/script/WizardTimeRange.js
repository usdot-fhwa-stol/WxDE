...
function convertToLongUTC(dDate) {
	var hoursOffset = dDate.getTimezoneOffset() / 60;
	return (dDate.getTime() - (3600000 * hoursOffset));
...
function Validate()
{
    ...
    var start = new Date(startDate + " " + startTime + ":00:00");
    
    var endDate = document.getElementById("endDate").value;
    var endTime = document.getElementById("endTime").value;
    var end = new Date(endDate + " " + endTime + ":00:00");
    
    var lStart = convertToLongUTC(start);
    var lEnd = convertToLongUTC(end);

/*
    console.log("lStart->" + lStart);
    console.log("lEnd->" + lEnd);
    console.log($('input:radio[name=docFormat]:checked').val());
 */
    ...
    if (lStart != lEnd)
    {
        ...
        oFormat.value = $('input:radio[name=docFormat]:checked').val();

/***
        if (!window.confirm('The user assumes the entire risk related to the use of data contained in the Weather Data Environment.\n' +

...
public class Stage24Precip implements Runnable
{
	...
	private Stage24Precip()
	{
		...
		Scheduler.getInstance().schedule(this,
			m_nOffset * 60, oConfig.getInt("period", 60) * 60);
		...
		double dRadius, int nHours)
	{
		...
		try
		{
			...
			{
				Log.getInstance().write(this, "getPrecipAccum",
					Double.toString(dLat), Double.toString(dLon),
					Integer.toString(m_nLimitX[0]), Integer.toString(m_nLimitX[1]),
					Integer.toString(m_nLimitY[0]), Integer.toString(m_nLimitY[1]),
					Integer.toString(nWidth), Integer.toString(nHeight));
					...
					while (nIndexH-- > 0)
					{
						...
						else
						{
							...
							if (Double.isNaN(dValue) ||
								Double.isNaN(dValues[nIndexY][nIndexX]))
							{
								dValues[nIndexY][nIndexX] = Double.NaN;
							}
							else
								dValues[nIndexY][nIndexX] += dValue;
						}
					}
				}
			}

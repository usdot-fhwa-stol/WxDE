...
public class QChSeq implements Comparable<QChSeq>
{
	...
	private static String QCHSEQ_QUERY = "SELECT seq, bitPosition, " + 
		...
		"FROM qchseq WHERE qchseqmgrId = ?";

	/**
	 * Seperate quality check sequences by climate. Helps enforce an ordering.
	 */
	...
	QChSeq(int nSeqMgrId, int nClimateId, Connection iConnection)
	{
		...
		try
		{
			...
			PreparedStatement iPreparedStatement = 
				iConnection.prepareStatement(QCHSEQ_QUERY);
			iPreparedStatement.setInt(1, nSeqMgrId);
			...
			{
				QCh oQCh = 
					(QCh)Class.forName(iResultSet.getString(6)).newInstance();
	...
	void check(int nObsTypeId, ISensor iSensor, IObs iObs, QChResult oResult)
	{
		...
		iObs.setFlags(nRun, nFlags, (float)dConfidence);
	}

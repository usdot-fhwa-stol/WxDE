 ...
 * sensor, of all the surrounding sensors on the same station. Then determines
 ...
 * other similar instruments in the area, on the same station for its weighted
 * mean calculation.
 * </p>
 */
public class Barnes extends LikeInstrument
{
	...
	protected static int sphereAdjust(int nRadius, int nLat)
	{
		...
		if (dRatio1 > dRatio2)
			dRatio1 = dRatio2;
		return Stations.toMicro(Stations.fromMicro(nRadius) / dRatio1);
	}
		...
		ArrayList<IObs> oObsSet, ModObsSet oModObsSet)
	{
		...
		// for this comparison to avoid finding each station explicitly
		...
		while (nIndex-- > 0)
		{
			...
			// exclude the target sensor, but include station sensors
		...
		IObs iObs, QChResult oResult)
	{
		...
		// determine how many stations fall within +/- 350 m elevation
		int nObsCount = 0;
		int nElevMax = iObs.getElev() + 350;
		int nElevMin = iObs.getElev() - 350;
		double dSum = 0.0;
		while (nIndex-- > 0)
		{

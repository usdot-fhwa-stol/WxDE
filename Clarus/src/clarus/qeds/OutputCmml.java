...
{
    /**
     * Quality checking algorithm names, matches up with run, and pass
     * bit-fields.
     */
	private String[] m_sQch =
	{
		"Complete", "Manual", "Sensor_Range", "Climate_Range", "Step",
		"Like_Instrument", "Persistence", "IQR_Spatial", "Barnes_Spatial",
		"Dew_Point", "Sea_Level_Pressure", "Precip_Accum"
	};
		...
		Subscription oSub, String sFilename, int nId, long lLimit)
	{
        ...
        try
        {
			...
			if (sFilename != null)
			{
				...
				addElement(oWriter, "title", "Clarus System Observations");
			...
			addElement(oWriter, "production-center", "Mixon/Hill, Inc.");
			...
			for (int nIndex = 0; nIndex < oSubObsList.size(); nIndex++)
            {
				...
				int nCmmlIndex = Collections.
					binarySearch(m_oCmmlMapping, oCmmlTypeSearch);

				if (nCmmlIndex < 0)
					continue;

				CmmlType oCmmlType = m_oCmmlMapping.get(nCmmlIndex);
				IStation iStation = oSubObs.m_iStation;

                if (iStation.getId() != nStationId)
                {
                    if (nStationId > 0)
                        endTag(oWriter); // observation-series

                    nStationId = iStation.getId();
                    ...
                    addElement(oWriter, "id", Integer.toString(nStationId), 
						"type", "station-id");

                    addElement(oWriter, "id", iStation.getCode(), 
						"type", "station-code");

                    addElement(oWriter, "id",
						Integer.toString(iStation.getSiteId()),
						"type", "site-id");

                    addElement(oWriter, "id", 
						Character.toString(iStation.getCat()),
						"type", "category");
                ...
                addElement(oWriter, "value", 
					...
					"units", oSubObs.m_iObsType.getUnit());
                ...
                addElement(oWriter, "qualifier",
					Integer.toString(iSensor.getSensorId()),
					"type", "sensor-id");

                addElement(oWriter, "qualifier", 
					m_oDecimal.format(oSubObs.m_dEnglishValue),
					"type", "english-value",
					"units", oSubObs.m_iObsType.getEnglishUnit());
	...
	protected void outputQch(PrintWriter oWriter, int nRunFlags, int nPassFlags)
	{
		...
		// ouput "Passed", "Not passed", "Not run", "Not applicable" states
		int nQchIndex = 0;
		int nIndex = getQchLength();
		while (nIndex-- > 0)
		{
			startTag(oWriter, "qch", "performer", m_sQch[nQchIndex++]);

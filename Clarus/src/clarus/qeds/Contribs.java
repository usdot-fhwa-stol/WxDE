...
public class Contribs extends DbCache<Contrib>
{
	...
	private Contribs()
	{
		...
		m_sQuery = "SELECT id, name, monitorHours, display FROM contrib";
...
//		oPrintWriter.println("</td></tr>");
     ...
     * @param iResultSet database query result containing the {@code Contrib}
     * data.
     * @param oContrib object whose parameters are to be set.
     */
	protected void setParameters(ResultSet iResultSet, Contrib oContrib)
	{
		try
		{
			oContrib.m_nId = iResultSet.getInt(1);
			oContrib.m_sName = iResultSet.getString(2);
			oContrib.m_nHours = iResultSet.getInt(3);
			oContrib.m_nDisplay = iResultSet.getInt(4);
		}

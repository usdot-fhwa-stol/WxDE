...
public class MappedValues extends DbCache<MappedValue>
{
	...
	private MappedValues()
	{
		...
		m_sQuery = "SELECT obsTypeId, valueLabel, value FROM obsvaluemap";
...
//		oPrintWriter.println("</td></tr>");
     ...
     * </p>
     *
     * @param iResultSet The {@code ResultSet} with which to retrieve the data.
     ...
     * set.
     */
	protected void setParameters(ResultSet iResultSet, MappedValue oMappedValue)
	{
		try
		{
			oMappedValue.m_nObsType = iResultSet.getInt(1);
			oMappedValue.m_sLabel = iResultSet.getString(2); 
			oMappedValue.m_dValue = iResultSet.getDouble(3); 
		}
	...
	/**
     * Creates a key-value pair from the given label and obseravation
	...
	public double getValue(int nCollectorId, int nObsType, CharSequence sLabel)
	{
		...
		if (oRecord == null)
		{
			...
			synchronized(m_oMissingValues)
			{
				...
				if (nIndex < 0)
				{
					m_oLog.write(this, "getValue",
						Integer.toString(nCollectorId),
						Integer.toString(nObsType), sLabel);
	...
	private class CollectorValue implements Comparable<CollectorValue>
	{
         ...
         * <b> Default Constructor </b>
		 * <p>
		 * Creates new instances of {@code CollectorValue}
		 * </p>
         */
		private CollectorValue()
		{
		}
